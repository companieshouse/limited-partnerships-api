...
locals {
  ...
  service_name               = "limited-partnerships-api"
  container_port             = "8080"
  eric_port                  = "10000"
  docker_repo                = "limited-partnerships-api"
  kms_alias                  = "alias/${var.aws_profile}/environment-services-kms"
  lb_listener_rule_priority  = 23
  lb_listener_paths          = ["/transactions/*/incorporation/limited-partnership/*", "/transactions/*/limited-partnership/*", "/limited-partnership/healthcheck"]
  healthcheck_path           = "/limited-partnership/healthcheck" #healthcheck path for limited-partnerships-api
  healthcheck_matcher        = "200"
  vpc_name                   = local.service_secrets["vpc_name"]
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "limited-partnerships-api.env"
